...
export default function PosterEditorTool() {
  ...
  const [shapeIntensity, setShapeIntensity] = useState(180);
    ...
    <main className="min-h-screen bg-zinc-100 p-2 md:p-6 lg:p-8 font-quicksand overflow-x-hidden">
      ...
      <div className="mx-auto grid max-w-7xl gap-4 lg:grid-cols-[430px_1fr]">
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-zinc-200 p-4 bg-zinc-50/50">
              ...
              <div className="space-y-4">
                ...
                {bgBottomShape !== 'flat' && (
                  <Range label="Độ nghiêng / Uốn cong" value={shapeIntensity} min={0} max={300} step={5} onChange={setShapeIntensity} />
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-4 rounded-xl border border-zinc-200 p-4 bg-zinc-50/50">
              ...
              <ColorInput label="Màu nền thẻ" value={cardColor} onChange={setCardColor} />
        ...
        <section className="rounded-2xl bg-white p-3 shadow-sm md:p-4 overflow-hidden flex flex-col items-center justify-start">
          ...
          <div
            className="relative shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-zinc-200 bg-white transition-all duration-200 ease-out"
...
function PosterPreview({ data, setImgTransform, previewScale }: { 
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#fcf9f2] font-quicksand">
      ...
      <div 
        ref={imgBoxRef}
        className="absolute z-10 overflow-hidden rounded-[40px] bg-[#f3efe4] shadow-[0_15px_40px_rgba(0,0,0,0.15)] group touch-none"
        style={{ 
          left: L.imageX, top: L.imageY, width: L.imageW, height: L.imageH, cursor: 'grab',
          borderWidth: showBorder ? '6px' : '0px', 
      ...
      <div 
        className="absolute z-10 rounded-[40px] flex flex-col px-[65px] py-[40px] shadow-[0_10px_30px_rgba(0,0,0,0.08)]"
        style={{ 
      ...
      >
        <div className="text-center flex flex-col items-center">
          ...
          <div className="mt-1 text-[56px] font-bold leading-tight font-cute" style={{ color: data.themeColor }}>{data.packageTitle}</div>
          {data.studioName && (
            <div className="mt-3 text-[22px] font-bold uppercase tracking-[0.2em] text-zinc-600" style={{ color: data.themeColor }}>{data.studioName}</div>
          )}
        </div>
      ...
      <div className="absolute left-0 right-0 z-20 flex flex-col items-center text-[26px] font-bold" style={{ top: 1490, color: data.textColor }}>
...
async function drawPosterCanvas(ctx: CanvasRenderingContext2D, data: PosterData) {
  ...
  if (data.photo) {
    ...
    ctx.translate(L.imageX + L.imageW / 2, L.imageY + L.imageH / 2);
    ctx.translate(data.imgTransform.x, data.imgTransform.y);
    ctx.scale(data.imgTransform.scale, data.imgTransform.scale);
    ctx.drawImage(img, -drawW / 2, -drawH / 2, drawW, drawH);
  }
  ctx.restore();

  if (data.imageBorderColor && data.imageBorderColor !== 'transparent') {
    ctx.save();
    roundRect(ctx, L.imageX, L.imageY, L.imageW, L.imageH, L.imageR);
    ctx.lineWidth = 12; 
  ...
  if (data.studioName) {
    ...
    ctx.fillText(data.studioName.toUpperCase(), w/2, L.cardY + 230);
  ...
  ctx.fillText(`📍`, w / 2 - ctx.measureText(`  ${data.address}`).width / 2 - 10, 1515);
  ctx.fillStyle = data.textColor;
  ctx.fillText(`${data.address}`, w / 2, 1515);
  const line2 = `f   ${data.facebook}     |     ☎   ${data.phone}`;
  ctx.fillText(line2, w / 2, 1560); 
}
...
function drawCanvasSection(
  ...
  ctx.textAlign = "left";
  ctx.fillStyle = color;
  ctx.font = "700 48px 'Dancing Script', cursive, sans-serif";
...
function PosterSection({ title, color, textColor, items }: { title: string, color: string, textColor: string, items: string[] }) {
  return (
    <div className="text-left font-quicksand">
      <h2 className="text-[48px] font-cursive leading-none" style={{ color }}>{title}</h2>
      <ul className="mt-2 space-y-1.5 text-[25px] font-bold leading-snug pl-2" style={{ color: textColor }}>
        {items.map((item, idx) => (
